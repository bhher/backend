...
const handlePhotoChange = (e) =>{
  ...
  return (
	<div>
	  <h3 className='alert alert-warning'>ThreeApp</h3>
	  <div className='d-inline-flex'>
		<select style={{width:'100px'}} className='form-control' 
		onChange={(e)=> handlePhoneChange(e, 'hp1')} value={phone.hp1}>
			<option>010</option>
			<option>02</option>
			<option>031</option>
			<option>042</option>
		...
		placeholder='0000' 
		onChange={(e)=> handlePhoneChange(e, 'hp2')} value={phone.hp2}/>
		&nbsp;&nbsp;<b>-</b>&nbsp;&nbsp;
		<input type='text' maxLength={4} className='form-control' style={{width:'120px'}} 
		placeholder='0000' 
		onChange={(e)=> handlePhoneChange(e, 'hp3')} value={phone.hp3}/>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<b>이미지 선택 : </b>&nbsp;&nbsp;
		<select className='form-control' style={{width:'100px'}} 
		onChange={handlePhotoChange} value={photoIndex}>
	  ...
	  <br/>
	  <hr/>
	  <h1>{phone.hp1} - {phone.hp2} - {phone.hp3}</h1>
	  <br/>
	  <img src={images[photoIndex -1 ]} alt={`선택된이미지 ${photoIndex}`} 
	  style={{maxWidth:'100%'}}/>
	</div>
  )
}

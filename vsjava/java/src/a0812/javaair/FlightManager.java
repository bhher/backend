...
public class FlightManager {
    ...
    //승객이름을 키로 하고 , 예약된 항공편을 값으로 가지는 Map
    ...
    public void displayFlightList(String str) {
        //항공편 목록 출력 -> flight에서 toString  Override 
    ...
    public void bookFlight() throws InterruptedException {//항공권 예매

        //국제선 - 사용자 이름, 생년월일 받아서 나이 확인하고 만 15세미만 예약거절
        for(;;){//무한루프
            displayFlightList("항공편 예매"); //항공편 목록
    ...
    private void passengerInfo(Flight flight) { //사용자 입력해서 예약
        ...
        try{
            ...
            if(!p.man15(p) && flight.getInternationalFlight()){
                //왼쪽과 오른쪽씩 이 true 여야만 실행
                //만 15세 나이 검증
                System.out.println("만 15세 미만이므로 국제선 예약 불가합니다.");
            }else{

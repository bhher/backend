...
for(int i =0; i < englishScores.length;i++){ //반의 수만큼 반복
    totalStudent = totalStudent + englishScores[i].length; //총 학생수 5 - 반의 학생수 합산
    for(int k = 0; k < englishScores[i].length;k++){ //해당반의 학생 수 만큼 반복
       totalEnglishSum = totalEnglishSum + englishScores[i][k]; //학생점수 합산
    }
}
